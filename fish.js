...
class Fish {
    ...
    _avoid_obstacles(obstacles, camera_pos) {
        ...
        const repel = (px, py, pz, obj_radius) => {
            ...
            const pen = raw - obj_radius; // penetration distance into buffer zone
     ...
     * @param {Fish[]}  neighbors     fish in same school
    ...
    update(dt, neighbors, obstacles, camera_pos, play_bounds) {
        ...
        // Steering forces (Reynolds' rules)
     ...
     * @param {Shape}         shape     shared Subdivision_Sphere
    ...
    draw(caller, uniforms, shape, material) {
        ...
        const seg1_root = base
            .times(Mat4.translation(0, 0, -s * 0.80))  // move to body's rear joint
            .times(Mat4.rotation(wave1, 0, 1, 0));      // bend the tail

        shape.draw(caller, uniforms,
            seg1_root
                .times(Mat4.translation(0, 0, -s * 0.30)) // draw at segment midpoint
        ...
        const seg2_root = seg1_root
            .times(Mat4.translation(0, 0, -s * 0.60))
            .times(Mat4.rotation(wave2, 0, 1, 0));

        shape.draw(caller, uniforms,
            seg2_root
                .times(Mat4.translation(0, 0, -s * 0.20))
                .times(Mat4.scale(s * 0.22, s * 0.18, s * 0.32)),
            mat);

        // ── TAIL FIN (flattened: wide in X, thin in Y) ────────────────────
        const wave3    = Math.sin(t * f + 2 * ph) * ta;
        const fin_root = seg2_root
            .times(Mat4.translation(0, 0, -s * 0.40))
            .times(Mat4.rotation(wave3, 0, 1, 0));

        shape.draw(caller, uniforms,
            fin_root
                .times(Mat4.translation(0, 0, -s * 0.12))
                .times(Mat4.scale(s * 0.48, s * 0.06, s * 0.20)),
            mat);

        // ── LEFT PECTORAL FIN ─────────────────────────────────────────────
        const fin_wave = Math.sin(t * f * 0.8) * fa;

        shape.draw(caller, uniforms,
            base
                .times(Mat4.translation(-s * 0.50, -s * 0.05, 0))
                .times(Mat4.rotation(-fin_wave, 0, 0, 1))  // flap up/down around Z
                .times(Mat4.scale(s * 0.48, s * 0.07, s * 0.30)),
            mat);
        ...
        shape.draw(caller, uniforms,
            base
                .times(Mat4.translation(0, s * 0.30, -s * 0.10))
...
class Fish_School {
     ...
     * @param {Vector4} cfg.fish_color  RGBA color for all fish in school
...
export class Fish_Manager {
    ...
    constructor(obstacles = []) {
        ...
        this.schools = [
            ...
            {
                // Yellow school — deeper, heading +X
